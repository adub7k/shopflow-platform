    ...
    const calls = (l.calls||[]).map(c => {
      ...
      if (va && va.turns && va.turns.length) {
        ...
        if (va.trace && va.trace.length) {
          ...
          brain = `<div style="margin-top:6px;"><button onclick="event.stopPropagation();const p=this.nextElementSibling;p.style.display=p.style.display==='none'?'block':'none';" style="padding:4px 10px;border:1px solid var(--border);background:var(--surface);color:var(--text);border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;">🧠 Brain · ${va.trace.length} turns · avg ${Math.round(total.ms/va.trace.length)} ms · ${total.in+total.out} tokens${total.cached?` (${Math.round(100*total.cached/Math.max(1,total.in+total.cached))}% cached)`:''}${nFix?` · ${nFix} corrected`:''}${nGuard?` · <span style="color:#dc2626;">${nGuard} guard</span>`:''}</button><div style="display:none;margin-top:4px;font-size:11px;">${rows}</div></div>`;
        }
