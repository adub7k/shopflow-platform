// ── PIN Auth ──────────────────────────────────────────────────────────────────
// Named PinAuth to avoid collision with the JWT Auth object in api.js
const PinAuth = {
  _unlocked: false,
  SESSION_KEY: 'sf_auth',
  ...
  showLock() {
    ...
    el.innerHTML = `
      <div style="text-align:center;max-width:300px;width:100%;padding:24px;">
        <img src="/icons/icon-192.png" style="width:80px;height:80px;border-radius:18px;margin:0 auto 16px;display:block;" />
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;max-width:220px;margin:0 auto;">
          ${[1,2,3,4,5,6,7,8,9,'',0,'⌫'].map(n=>`<button onclick="PinAuth._key('${n}')" style="background:${n===''?'transparent':'#f9fafb'};border:${n===''?'none':'1px solid #e5e7eb'};border-radius:10px;padding:16px 0;font-size:${n==='⌫'?'18px':'20px'};font-weight:600;color:#111;cursor:${n===''?'default':'pointer'};font-family:inherit;${n===''?'pointer-events:none;':''}" ${n===''?'disabled':''}>${n}</button>`).join('')}
        </div>
        <div style="margin-top:16px;">
          <button onclick="PinAuth.showResetPIN()" style="background:none;border:none;font-size:12px;color:#9ca3af;cursor:pointer;text-decoration:underline;font-family:inherit;">Forgot PIN?</button>
        </div>
        <div style="margin-top:8px;font-size:11px;color:#9ca3af;">Powered by <strong style="color:#16a34a;">ShopFlow</strong></div>
      </div>`;
    document.body.appendChild(el);
    this._pin = '';
    // Show shop name from localStorage (set at login)
    const shopName = localStorage.getItem('sf_shopName');
    const el2 = document.getElementById('pin-shop-name');
    if (el2 && shopName) el2.textContent = 'Welcome to ' + shopName;
  },
  ...
  async _verify() {
    try {
      const data = await db.auth.verifyPin(this._pin);
      if(data.ok){
        this._unlocked=true;
  ...
  showChangePIN() {
    Modal.show(`
      ...
      <div class="modal-actions">
        <button class="btn btn-green btn-full" onclick="PinAuth.savePin()">Save PIN</button>
        <button class="btn btn-full" onclick="PinAuth.showResetPIN()">Forgot current PIN?</button>
  ...
  showResetPIN() {
    Modal.show(`
      ...
      <div class="form-group"><label class="form-label">Account Email</label>
        <input class="form-input" id="rp-email" type="email" placeholder="Email used to sign up" />
      </div>
      ...
      <div class="modal-actions">
        <button class="btn btn-green btn-full" onclick="PinAuth.doResetPIN()">Reset PIN</button>
        <button class="btn btn-full" onclick="Modal.close()">Cancel</button>
      </div>`);
    setTimeout(()=>document.getElementById('rp-email')?.focus(),150);
  },
  ...
  async doResetPIN() {
    ...
    if(!email||!key||!nw||!con){toast('Fill in all fields','warning');return;}
    if(nw!==con){toast('PINs do not match','warning');return;}
    if(nw.length<4){toast('PIN must be 4+ digits','warning');return;}
    try{
      const res = await fetch('/api/shop/auth/reset-pin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ownerKey:key,newPin:nw,email})});
