// ── Demo seed: a self-contained cleaning company ─────────────────────────────
// Spins up a realistic small cleaning business so the cleaning vertical can be
// demoed/pitched: 1 crew, 5 recurring contracts, ~$15k/mo of completed work, and
// ~$7.5–8k/mo net profit. Mirrors seed-demo-detail.js: it creates its own shop +
// owner account, so it works on a fresh volume (e.g. Railway) with no shop set up.
//
//   node seed-cleaning-demo.js                 → wipe + recreate the demo (clean reset)
//
//   Login:  demo@summitcleaning.com / demo1234   (CRM at /shop/summit-cleaning)
//   Booking page:  /book/summit-cleaning
//
// Also runnable on boot: server/index.js calls this with { force:false } when the
// SEED_CLEANING=true env var is set, so a Railway deploy can seed its volume once
...
const { master, getShopDb, initShopDb, shopHelpers, today, SHOPS_DIR } = require('./server/db');

const SLUG      = 'summit-cleaning';
const SHOP_NAME = 'Summit Home Cleaning';
const EMAIL     = 'demo@summitcleaning.com';
const PASS      = 'demo1234';
const PHONE     = '(720) 555-0142';
const TARGET_MONTHLY = 15000;            // ~$15k/mo completed revenue

function seedCleaningDemo({ force = true } = {}) {
  const existing = master.get('shops').find({ slug: SLUG }).value();
  if (existing && !force) { console.log('• Cleaning demo already present — skipping seed'); return existing.id; }

  // Wipe any previous demo so re-runs are clean.
  if (existing) {
    ...
    console.log('• Removed previous cleaning demo');
  }

  // ── Register the tenant (account + shop), then init from the cleaning profile ─
  const shopId = uuidv4(), accountId = uuidv4();
  ...
  master.get('shops').push({
    id: shopId, accountId, shopName: SHOP_NAME, slug: SLUG, email: EMAIL,
    phone: PHONE, plan: 'shop', industry: 'cleaning', active: true,
    createdAt: new Date().toISOString(), lastActivity: new Date().toISOString(),
  }).write();
  const db = getShopDb(shopId);
  initShopDb(db, { shopName: SHOP_NAME, email: EMAIL, phone: PHONE, industry: 'cleaning' });
  ...
  const properties = allCustomers.map((c, i) => ({
    id: 'prop' + (i + 1), customerId: c.id, label: c.company ? 'Main location' : 'Home',
  ...
  const contracts = [
    { id: 'rec1', customerId: 'c1', serviceId: 's1', price: 185, cadence: 'weekly',   dow: 1, time: '8:00 AM'  },
    { id: 'rec2', customerId: 'c2', serviceId: 's4', price: 475, cadence: 'weekly',   dow: 5, time: '6:30 PM'  },
    { id: 'rec3', customerId: 'c3', serviceId: 's1', price: 210, cadence: 'biweekly', dow: 3, time: '9:00 AM'  },
    { id: 'rec4', customerId: 'c4', serviceId: 's4', price: 325, cadence: 'weekly',   dow: 2, time: '6:00 PM'  },
    { id: 'rec5', customerId: 'c5', serviceId: 's2', price: 300, cadence: 'biweekly', dow: 4, time: '10:30 AM' },
  ];
  const stepDays = (c) => c.cadence === 'biweekly' ? 14 : 7;
  ...
  // 1) Backfill recurring history (done) + set each contract's nextRunDate.
  contracts.forEach(c => {
    let d = WINDOW_START;
  ...
  const fillPool = [svc('s2'), svc('s3'), svc('s4'), svc('s5'), svc('s1')];
  ...
  // 4) Upcoming scheduled jobs (next ~10 days).
  contracts.forEach(c => {
  ...
  console.log(`\n✓ Cleaning demo seeded: "${SHOP_NAME}"`);
  console.log(`  CRM:     /shop/${SLUG}   login ${EMAIL} / ${PASS}`);
  console.log(`  Booking: /book/${SLUG}`);
  console.log(`  ${customers.length} customers · ${crews.length} crew · ${recurringServices.length} recurring · ${appts.length} jobs (${doneCount} completed)`);
  MONTHS.forEach(mk => console.log(`    ${mk}: $${(monthTotals[mk] || 0).toLocaleString()} completed`));
  console.log('');
  return shopId;
}

module.exports = seedCleaningDemo;

// Run directly (`node seed-cleaning-demo.js`) → force a clean reset.
if (require.main === module) seedCleaningDemo({ force: true });
